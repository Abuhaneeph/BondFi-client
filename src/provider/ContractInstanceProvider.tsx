...
const THIRDWEB_RPC_URL = `https://5003.rpc.thirdweb.com/${import.meta.env.VITE_THIRDWEB_CLIENT_ID}`;
...
export const ContractInstanceProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const isCorrectNetwork = activeChain?.id === MANTLE_CHAIN_ID;
    ...
    try {
      ...
      try {
        await window.ethereum.request({
          ...
          params: [{ chainId: `0x${MANTLE_CHAIN_ID.toString(16)}` }], // Convert 2810 to hex: 0xAFA
        });
        console.log('✅ Successfully switched to Mantle Sepolia network');
        return true;
      } catch (switchError: any) {
        // If network doesn't exist (error code 4902), add it
        if (switchError.code === 4902) {
          console.log('Mantle Sepolia network not found, adding it...');
          await window.ethereum.request({
            method: 'wallet_addEthereumChain',
            params: [
              {
                chainId: `0x${MANTLE_CHAIN_ID.toString(16)}`, // 0xAFA
          ...
          console.log('✅ Successfully added and switched to Mantle Sepolia network');
          return true;
        }
        throw switchError;
      }
        ...
        try {
          ...
          if (wallet.getChain && wallet.switchChain) {
            try {
              ...
              if (currentChain.id !== MANTLE_CHAIN_ID) {
          ...
          if (typeof window !== 'undefined' && window.ethereum) {
            ...
            if (Number(network.chainId) === MANTLE_CHAIN_ID) {
              ...
              console.warn('Wrong network detected:', network.chainId, 'expected:', MANTLE_CHAIN_ID);
              setNetworkError(`Wrong network. Expected chain ID: ${MANTLE_CHAIN_ID}, got: ${network.chainId}`);
            }
          }
  ...
  useEffect(() => {
    const handleNetworkChange = async () => {
      if (isConnected && activeChain?.id && activeChain.id !== MANTLE_CHAIN_ID) {
        console.log(`Wrong network detected: ${activeChain.id}, expected: ${MANTLE_CHAIN_ID}`);
        
        // Show network error immediately
        setNetworkError(`Connected to wrong network. Please switch to Mantle Sepolia (Chain ID: ${MANTLE_CHAIN_ID}) or click the switch button.`);
  ...
  const MULTICURRENCY_SAVING_CONTRACT_INSTANCE = async (): Promise<ethers.Contract | null> => {
   ...
   const MERCHANT_CORE_CONTRACT_INSTANCE = async (): Promise<ethers.Contract | null> => {
    ...
    const signerOrProvider = signer || provider;
    return new ethers.Contract(CONTRACT_ADDRESSES.merchantCoreInstallmentAddress, MERCHANT_CORE_ABI, signerOrProvider);
  };
    ...
    const signerOrProvider = signer || provider;
    return new ethers.Contract(CONTRACT_ADDRESSES.MERCHANT_REGISTRY, Merchant_Registry_ABI, signerOrProvider);
  };
    ...
    const signerOrProvider = signer || provider;
    return new ethers.Contract(CONTRACT_ADDRESSES.PRODUCT_CATALOG, Product_ABI, signerOrProvider);
  };
    ...
    const signerOrProvider = signer || provider;
    return new ethers.Contract(CONTRACT_ADDRESSES.INSTALLMENT_MANAGER, Installment_ABI, signerOrProvider);
  };
  ...
  const reconnectSigner = async (): Promise<void> => {
    try {
      ...
      if (!isCorrectNetwork) {
        throw new Error(`Wrong network. Please switch to chain ID: ${MANTLE_CHAIN_ID}`);
      }
