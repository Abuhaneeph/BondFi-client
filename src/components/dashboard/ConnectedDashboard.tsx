        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 border border-emerald-200 rounded-3xl p-8">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h3 className="font-medium text-emerald-800">Community Reputation</h3>
                <p className="text-sm text-emerald-600">On-chain trust score</p>
              </div>
            </div>
            <div className="flex items-center space-x-2 mb-2">
              <div className="text-3xl font-bold text-emerald-700">{communityStats.reputationScore}</div>
              <div className="flex items-center space-x-1">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < Math.floor(communityStats.reputationScore/2) ? 'text-yellow-500 fill-current' : 'text-slate-300'}`} />
                ))}
              </div>
            </div>
            <div className="text-xs text-emerald-600">{communityStats.communityTrust}% community trust</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
          <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-6">
            <div className="flex items-center space-x-3 mb-2">
              ...
              <div>
                <div className="text-2xl font-bold text-slate-800">₦{(120000).toLocaleString()}</div>
